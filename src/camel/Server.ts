...
export default class Server {
	...
	private PointCalc = async () => {
		...
		for(let i = 0; i < this.PLAYER_NUM; ++i) {
			...
			db.ref(PATH + 'player/point/diff').set(pointDiff);
			// this.running(this.PLAYER_NAME[i] + ': ' + pre + 'pt -> ' + this.playerPoint[i] + 'pt');
			await Sleep(2000);
		}
		for(let i = 0; i < this.PLAYER_NUM; ++i) {
			this.playerPoint[i] += pointDiff[i];
		}
		db.ref(PATH + 'player/point/val').set(this.playerPoint);
		db.ref(PATH + 'player/point/diff').set(new Array(this.PLAYER_NUM));
	};
	...
	public start = async (PLAYER_NAME: string[]) => {
		...
		db.ref(PATH + 'player/').set({
			...
			point: {val: this.playerPoint, diff: null},
